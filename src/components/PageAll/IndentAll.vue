...
export default {
  ...
  methods: {
    ...
    getMoreDataList() {
      if (this.page > 0) {
        ...
        this.$axios.post(url, { "page": this.page, "size": this.pageSize, "status": this.status })
          .then((res) => {
            this.total = res.data.proOrderAmount
            for (let i = 0; i < res.data.proOrder.length; i++) {
              this.arr.push(res.data.proOrder[i])
            }
            this.pageNum = res.data.proOrder.length;
            if (res.data.proOrder.length === this.pageSize) {
              this.page += 1
    ...
    goPayNent(took) { // 去结算
      var orderFrom = this.arr[took].paymentType;
      if (orderFrom == '') {
        return;
      } else {
        if (orderFrom == 1) { // 
          if (this.flage = true) {
            this.orderStringNo = this.arr[took].orderString;
            $(".password").fadeIn(300);
            var flage = false;
          }
        }
      ...
      if (orderFrom == 2) { // 支付宝
        var orderStringAli = this.arr[took].orderString;
        var val = {
          "func": 'pay',
          "param": {
            "payType": 'Ali',
      ...
      if (orderFrom == 3) { // wx
        var orderStringWx = this.arr[took].orderString;
        var val = {
          "func": 'pay',
          "param": {
            "payType": 'Wechat',
